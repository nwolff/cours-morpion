...
def obtenir_meilleur_coup(plateau: list[str], joueur: str) -> ConseilMinimax:
    ...
    for enfant in sorted(racine.enfants, key=lambda n: -n.score):
        label = (
            "victoire"
            if enfant.score > 0
            else ("nul" if enfant.score == 0 else "défaite")
        )
...
def construire_arbre(plateau: list[str], joueur: str) -> NoeudMinimax:
    racine = NoeudMinimax(
        plateau=plateau[:], position_jouee=None, score=0, est_max=True
    )
...
def _construire_sous_arbre(
    ...
    for i in range(9):
        if p[i] == " ":
            p[i] = joueur_courant
            enfant = NoeudMinimax(
                plateau=p[:], position_jouee=i, score=0, est_max=not est_max
            )
            noeud.enfants.append(enfant)
            score = _construire_sous_arbre(
                enfant, p[:], not est_max, joueur, profondeur + 1
            )
            enfant.score = score
            scores.append(score)
            p[i] = " "
